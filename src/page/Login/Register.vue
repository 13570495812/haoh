...
<script>
  import { sendMessaEx, keepConn, add2 } from "../../api/index.js";
  // 请提示
  import Vue from "vue";
  ...
  export default {
    ...
    methods: {
      ...
      gain() {
        ...
        if (this.register == "") {
          ...
          // ----
          sendMessaEx(params).then(res => {
            console.log(res);
            if (res.c == "2") {
              this.$toast("今天发送次数过多");
            } else if (res.c == "8") {
              this.$toast("在80秒之后再次发送");
            }
          });
        }
      },
      tiao() {
        if (this.openid) {
          let params = {
            phoneCode: this.verification,
            userName: this.register,
            ymhaohanToken: this.uuid,
            openid: this.openid
          };
          add2(params).then(res => {
            if (res.err == "OK") {
    ...
    mounted() {
      ...
      this.Invitation='http://shop.ymhaohan.cn/?from=singlemessage#/registere?7556f71559a4fd0be6343ecf3ccf553'
      this.Invitation=this.Invitation.split('?')[2]
    }
  };
</script>
